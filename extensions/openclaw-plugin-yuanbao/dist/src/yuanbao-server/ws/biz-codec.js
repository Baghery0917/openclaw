...
export function encodeBizPB(key, value) {
    try {
        ...
        createLog('biz-codec').error(`编码失败: key=${key}, error=${error.message}`);
        return null;
    }
}
...
export function encodeSendC2CMessageReq(data) {
    ...
    log.debug(`[msg-trace] encode c2c outbound: traceId=${data.trace_id ?? '(none)'}, msgSeq=${data.msg_seq ?? '(none)'}, to=${data.to_account}`);
...
export function encodeSendGroupMessageReq(data) {
    const logExt = toProtoLogExt(undefined, data.trace_id);
    const log = createLog('biz-codec');
    log.debug(`[msg-trace] encode group outbound: traceId=${data.trace_id ?? '(none)'}, msgSeq=${data.msg_seq ?? '(none)'}, groupCode=${data.group_code}`);
...
export function decodeInboundMessage(data) {
    ...
    const seqId = decoded.msgSeq !== undefined && decoded.msgSeq !== null
        ? String(decoded.msgSeq)
        : undefined;
    const log = createLog('biz-codec');
    log.debug(`[msg-trace] decoded inbound: traceId=${traceId ?? '(none)'}, seqId=${seqId ?? '(none)'}, from=${decoded.fromAccount || '?'}, msgId=${decoded.msgId || '?'}`);
    return {
