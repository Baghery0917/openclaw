...
- 定时提醒投递地址: channel=adp-openclaw, to=${cronDeliveryTarget}`;
              ...
              // Map: COS base path (without query params) -> correct full URL with valid signature
              // Built incrementally as upload results arrive, used to fix LLM-corrupted URLs
              const correctUrlMap = new Map<string, string>();
              
              // Shared helper: replace any LLM-generated URLs that share the same base path
              // but have corrupted query parameters (e.g. %3B→%3A signature hallucination)
              const fixCorruptedUrls = (text: string): string => {
                if (correctUrlMap.size === 0) return text;
                let fixed = text;
                for (const [basePath, correctUrl] of correctUrlMap) {
                  const escaped = basePath.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
                  // Match the base path followed by any query string (stops at whitespace, ), or ])
                  const pattern = new RegExp(escaped + "\\?[^\\s)\\]]*", "g");
                  const before = fixed;
                  fixed = fixed.replace(pattern, correctUrl);
                  if (fixed !== before) {
              ...
              const sendOutboundEnd = (text: string) => {
                ...
                if (pendingUploadResults.length > 0) {
                  ...
                  finalText = fixCorruptedUrls(finalText);
              ...
              await runtime.channel.reply.dispatchReplyWithBufferedBlockDispatcher({
                ...
                replyOptions: {
                  ...
                  onPartialReply: async (payload: { text?: string }) => {
                    ...
                    // Fix any LLM-corrupted COS URLs in the cumulative text before
                    // computing the delta, so streaming chunks carry correct URLs.
                    const fullText = fixCorruptedUrls(rawText);
